                                  ...
                                  float *ToLinearF)
{
    ...
    if (n >= stride)
    {
        ...
        else
        {
            ...
            while (n > 0)
            {
                REPEAT(stride, wp[stride] = (uint16_t)(wp[stride] + *wp); *op = ToLinearF[*wp & mask];
                       wp++; op++)
                n -= stride;
            }
        }
...
#define CLAMP12(t) (((t) < 3071) ? (int16_t)(uint16_t)(t) : (int16_t)3071)
    ...
    if (n >= stride)
    {
        ...
        else
        {
            ...
            while (n > 0)
            {
                REPEAT(stride, wp[stride] = (uint16_t)(wp[stride] + *wp);
                                   ...
                                   uint16_t *op, uint16_t *ToLinear16)
{
    ...
    if (n >= stride)
    {
        ...
        else
        {
            ...
            while (n > 0)
            {
                REPEAT(stride, wp[stride] = (uint16_t)(wp[stride] + *wp); *op = ToLinear16[*wp & mask];
                       wp++; op++)
                n -= stride;
            }
        }
                                   ...
                                   uint16_t *op)
{
    ...
    if (n >= stride)
    {
        ...
        else
        {
            ...
            while (n > 0)
            {
                REPEAT(stride, wp[stride] = (uint16_t)(wp[stride] + *wp); *op = (uint16_t)(*wp & mask); wp++; op++)
                n -= stride;
            }
        }
    }
                                  ...
                                  unsigned char *op, unsigned char *ToLinear8)
{
    ...
    if (n >= stride)
    {
        ...
        else
        {
            ...
            while (n > 0)
            {
                REPEAT(stride, wp[stride] = (uint16_t)(wp[stride] + *wp); *op = ToLinear8[*wp & mask];
                       wp++; op++)
                n -= stride;
            }
        }
...
static int PixarLogSetupDecode(TIFF *tif)
{
    ...
    tbuf_size = add_ms(tbuf_size, (tmsize_t)((size_t)sizeof(uint16_t) * sp->stride));
    if (tbuf_size == 0)
...
static int PixarLogDecode(TIFF *tif, uint8_t *op, tmsize_t occ, uint16_t s)
{
    ...
    llen = (int)(sp->stride * td->td_imagewidth);
